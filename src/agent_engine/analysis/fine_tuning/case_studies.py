...
from .runs import ROOT, latest_run
...
def main():
    """Print the case studies.

    Body lifted unchanged from this module's ``__main__`` block, so that the
    CLI shim at ``scripts/failure_modes/fine_tuning/case_studies.py`` has a
    callable entry point.  Running the module directly behaves as before.
    """
