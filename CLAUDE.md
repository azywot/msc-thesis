...
- **BigCodeBench**: split is a version string (e.g. `v0.1.4_subset_200`); the `test` field is a `unittest.TestCase` class run via `unittest.main`; `return_code: true` must be set so the tool returns code rather than executing it; download via `python scripts/download_datasets.py --dataset bigcodebench --split v0.1.4 --subset 200`
